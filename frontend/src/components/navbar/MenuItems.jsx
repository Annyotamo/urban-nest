...
const MenuItems = () => {
    ...
    if (logout) return <ErrorOverlay close={true} closeFunc={() => setLogout(false)} action="Logout" actionFunc={handleLogout} message="Are you sure?" />
    ...
    return (
        <div>
            ...
            {data?.data.isAuthenticated === false && <MenuItem label='login' endpoint="/user/login" styles="" />}
            {data?.data.isAuthenticated === true && <MenuItem label='logout' endpoint="/" styles="border-t text-red-400" onClick={handleLogout} />}
